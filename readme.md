...
| 平台 | 网址 | 适用区域 |
|------|------|---------|
| ⭐ **主域名（推荐）** | https://namecard.store/ | 🌍 全球可用 |
| **公司NAS**（内网备用）| http://linpo.familyds.net:8080/card/ | 🇨🇳 国内随时可用 |
| **公司内网** | http://192.168.90.24/card/ | 🏢 公司网络内 |
| **自定义域名（WWW）** | https://www.namecard.store/ | 🌍 全球可用 |
...
> 💡 **推荐优先使用主域名 `https://namecard.store/`**
...
1. 打开网页：**https://namecard.store/**
2. **选择公司** → 从下拉列表选择你所在的公司
3. **选择版式** → 横版或竖版
4. **选择模板** → 9种精美模板任你选
5. **填写信息** → 输入姓名、职位、手机、邮箱等
6. **上传头像**（可选）→ 上传后可拖动照片调整位置，让人脸居中
...
| 科技蓝 | 蓝色渐变 | 科技、AI |
| 中国红 | 白底 + 红色 | 传统企业、国企 |
| 电路板 | 绿色电路纹理背景 | 电子、硬件、制造 |
| 星空科技 | 星空+连线图案 | AI、云计算、软件 |
| 几何蓝白 | 浅色几何图案 | 教育、咨询、医疗 |
| 紫韵波纹 | 紫色波纹背景 | 创意、设计、品牌 |
...
- 网址：https://namecard.store/
- 备用网址：https://www.namecard.store/
...
- ✅ 实时预览效果
- ✅ 导出高清PNG图片
- ✅ 支持上传个人头像（可拖动调整位置、缩放大小）
- ✅ 可选字段：微信、座机、部门
- ✅ **本地存储**：自动保存填写内容，刷新页面不丢失
- ✅ **二维码**：填入链接后名片自动显示二维码（支持网址/微信等）
- ✅ **导出提示**：导出完成后显示 Toast 提示
- ✅ 英文名片手机号自动规范化（+86 格式去横线）
- ✅ 模板缩略图升级（迷你名片样式预览）
- ✅ 竖版名片联系信息布局优化
- ✅ 名片背面装饰图案（支持开关 + 3种高级风格切换）
- ✅ 纯前端实现，无需后端服务器
- ✅ 响应式设计，手机电脑都能用
...
- 推荐填写：`https://你的官网`，扫码后可直接打开网页
- 也可以直接填微信号：例如 `zhangsan_wx`（扫码后显示文本，方便对方复制/搜索）
- 如果你填的是 `www.example.com`，系统会自动补成 `https://www.example.com`
- 微信个人主页通常没有公开网址，所以个人微信最实用的方式是直接填微信号

---

## 📝 后续可改进

- [x] 头像裁剪定位功能（拖动+缩放让人脸居中）
- [x] 本地存储自动保存/恢复表单
- [x] 名片二维码嵌入
- [x] 导出成功 Toast 提示
- [x] 英文名片手机号格式规范化
